  ...
  routes.overview = function () {
    ...
    return `
      ...
      ${(w.waiting && (pend || kyc || openD || highA)) ? `<div class="box" style="background:var(--gold-100);border-color:#EBD9AE;margin-bottom:18px">
        <div class="box__head" style="margin-bottom:0"><h3 class="h3">Waiting on you</h3>${hideBtn('waiting', 'the waiting-on-you banner')}</div>
      ...
      ${(w.chart || w.roles) ? `<div class="cards cols-2" style="margin-bottom:16px">
        ${w.chart ? `<section class="box">
          <div class="box__head"><h3>Money moved through the platform</h3>
            <span class="row" style="gap:10px"><span class="pcard__meta">last 6 months</span>${hideBtn('chart', 'the money-flow chart')}</span></div>
          <div class="chart">
        ...
        ${w.roles ? `<section class="box">
          <div class="box__head"><h3>Who is on the platform</h3>${hideBtn('roles', 'user mix and district funding')}</div>
      ...
      ${(w.tx || w.audit) ? `<div class="cards cols-2">
        ${w.tx ? `<section class="box">
          <div class="box__head"><h3>Latest transactions</h3>
            <span class="row" style="gap:10px"><a class="link" href="#/finance">Open finance</a>${hideBtn('tx', 'latest transactions')}</span></div>
          ${txTable(S.tx.slice(0, 5), true)}
        </section>` : ''}
        ${w.audit ? `<section class="box">
          <div class="box__head"><h3>Audit log</h3>
            <span class="row" style="gap:10px"><a class="link" href="#/reports">Full log</a>${hideBtn('audit', 'the audit log')}</span></div>
